...
Inherits ZipFolderItemBase
	...
	#tag Method, Flags = &h0
		Function StoreFile(inputFile as FolderItem, name as String = "", useMacBinary as Integer = ZipArchive.MacBinaryNever) As ZipFolderItem
	...
	#tag Method, Flags = &h0
		Function StoreFolderContents(folder as FolderItem, includeInvisibles as Boolean = true, useMacBinary as Integer = ZipArchive.MacBinaryNever, aliasHandling as ZipArchive.MacAliasHandling = ZipArchive.MacAliasHandling.StoreAll) As Boolean
	...
	#tag Method, Flags = &h0
		Function StoreFolderItem(fileOrFolder as FolderItem, includeInvisibles as Boolean = true, useMacBinary as Integer = ZipArchive.MacBinaryNever, aliasHandling as ZipArchive.MacAliasHandling = ZipArchive.MacAliasHandling.StoreAll) As ZipFolderItem
		  ...
		  if fileOrFolder.Directory then
		    ...
		    if fileOrFolder.Count > 0 then
		      if not folder.StoreFolderContents (fileOrFolder, includeInvisibles, useMacBinary, aliasHandling) then
		    ...
		    return me.StoreFile (fileOrFolder, "", useMacBinary)
		  end
		End Function
	#tag EndMethod

	#tag Method, Flags = &h0
		Function StoreStringAsFile(data as String, name as String, fileDate as Date = nil) As ZipFolderItem
		  ...
		  if super.prepareStore (name, rawPath) then
		    dim e_idx as Integer
		    e_idx = zar.AddItemByStreams (new StringReader (data), data.LenB, nil, 0, nil, rawPath, nil, fileDate)
		    if e_idx > 0 then
		      dim item as ZipFolderItem = me.createChild (name)
		      item.setEntry zar.Entry(e_idx)
		      return item
	...
	#tag Note, Name = Looking for Rename & Move?
		...
		If, however, you really need a Rename or Move function, I can add that on request,
		provided you're showing your support for this being shareware (hint hint).
